...
xi.spells.spell_damage.calculateBaseDamage = function(caster, target, spell, spellId, skillType, statDiff)
    ...
    if skillType == xi.skill.ELEMENTAL_MAGIC then
        if caster:isPC() then
			...
			if statDiff < 0 then
				statDiffBonus = 0
				--Debug
				--printf("baseSpellDamage = %s, statDiff = %s, spellMultiplier0 = %s, statDiffBonus = %s", baseSpellDamage, statDiff, spellMultiplier0, statDiffBonus)
            elseif statDiff < 50 then
	...
	--Debug
	--printf("baseSpellDamage = %s, baseSpellDamageBonus = %s, statDiffBonus = %s.", baseSpellDamage, baseSpellDamageBonus, statDiffBonus)
...
xi.spells.spell_damage.calculateResist = function(caster, target, spell, skillType, spellElement, statDiff, bonusMagicAccuracy)
    ...
    -----------------------------------
    local levelDiff =  utils.clamp(caster:getMainLvl() - target:getMainLvl(), -5, 5) * xi.settings.MOB_LEVEL_CORRECTION
...
xi.spells.spell_damage.useDamageSpell = function(caster, target, spell)
    ...
    -- Handle final adjustments. Most are located in core. TODO: Decide if we want core handling this.
        ...
        target:handleScarletDeliriumDamage(finalDamage)
